...
fun ComposeFlowLayout(
    ...
    fun getItemTop(gravity: Int, lineHeight: Int, topOffset: Int, childHeight: Int): Int {
        return when (gravity) {
    ...
    Layout(
        ...
        var totalHeight = 0
        var lineHeight = 0
        val mAllPlaceable = mutableListOf<MutableList<Placeable>>()  // 所有可放置的内容
        ...
        layout(parentWidthSize, totalHeight) {
            var topOffset = 0
            var leftOffset = 0
            for (i in mAllPlaceable.indices) {
                lineViews = mAllPlaceable[i]
