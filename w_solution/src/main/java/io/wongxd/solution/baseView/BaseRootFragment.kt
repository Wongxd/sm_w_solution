...
abstract class BaseRootFragmentLast : BaseRootFragment3(){

    // <editor-fold desc="状态栏">
    open val needSetStatusBarByBase = true

    open val setStatusBarLightMode = true

    open val insert2StatusBar = false

    open val statusBarColorRes = defaultBackgroundColorRes
            ...
            if (needSetStatusBarByBase) {
                statusBar
                    .color(
                        ...
                        else statusBarColor ?: ContextCompat.getColor(requireContext(),statusBarColorRes)
                    )
                    .light(setStatusBarLightMode)
                    .insert2StatusBar(insert2StatusBar)
                    .userFakeStatusBar(false)
                    .apply()
            }
        } catch (e: Exception) {
        }
    ...
    open override fun onInvisible() {
        hideSoftInput()
    }
}

abstract class BaseRootFragment3 : BaseRootFragment2() {

    abstract val layoutId: Int

    @ColorRes
    open val defaultBackgroundColorRes: Int = android.R.color.white

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        supportDelegate.defaultBackgroundColor = defaultBackgroundColorRes
    }
    ...
    open fun atySupportBack() {
        (requireActivity() as ISupportActivity).onBackPressedSupport()
    }

    fun startBrotherFgt(fgt: ISupportFragment) {
        (parentFragment as ISupportFragment?)?.supportDelegate?.start(fgt) ?: start(fgt)
    }
}

abstract class BaseRootFragment2 : BaseRootFragment1(), ISupportFragment {
    override val supportDelegate by lazy { SupportFragmentDelegate(this) }
    private var _mActivity: BaseRootActivityLast? = null
    ...
    override fun onAttach(context: Context) {
        ...
        _mActivity = supportDelegate.activity as BaseRootActivityLast
    }
    ...
    val topFragment: ISupportFragment?
        get() = SupportHelper.getTopFragment(requireFragmentManager())
    ...
    fun <T : ISupportFragment?> findFragment(fragmentClass: Class<T>?): T? {
        return SupportHelper.findFragment(requireFragmentManager(), fragmentClass)
    }
...
abstract class BaseRootFragment1 : Fragment()